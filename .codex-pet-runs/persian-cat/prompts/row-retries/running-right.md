Create Codex pet row `running-right` for `persian-cat`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: A compact full-body Persian cat with fluffy cream fur, round flat face, tiny ears, bushy tail, and large bright yellow eyes; soft sticker mascot style, no props.. Preserve silhouette, face, palette, material, proportions, markings, and props.
