Create Codex pet row `running-right` for `nina`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Nina, a compact full-body doll-faced Persian cat with plush grey fur, round open sweet face, small rounded ears, bushy tail, and large bright yellow eyes; soft sticker mascot style, no props.. Preserve silhouette, face, palette, material, proportions, markings, and props.
